    ...
    methods:{
        methodFilter(){
            ...
            if(this.filterValue.length>=3){
                this.stories=this.stories.filter(x=>{
                    let X=""
                    if(x.hiddenText){
                        X=x.hiddenText.toUpperCase()
                    }
                    if(x.text){
                        X+=x.text.toUpperCase()
                    }
                    if(X){
                        return X.includes(this.filterValue.toUpperCase())
                    }
                })
            }
        },
        ...
        calculateId(counter){
            const temp = '0000' + counter
            const id= temp.substring(temp.length-3,temp.length)
            return id
        }
    }
})
